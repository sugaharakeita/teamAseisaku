...
int CObjKateika::GetMap(int x, int y)
{
...
void CObjKateika::SetMap(int x, int y, int id)
{
	if (x < MAP_MIN)return;
	if (y < MAP_MIN)return;
...
void CObjKateika::Init()
{
	m_time = 0;
}

void CObjKateika::Action()
{
	m_time++;
	if (HeroX <= 0.0f)
		HeroX = 0.0f;
	...
	if (m_time == 5)
		Rouka2L = KateikaJunbi = 0;//SwitchALLÇÃç¨ìØÇîÇØÇÈÇΩÇﬂ
}
