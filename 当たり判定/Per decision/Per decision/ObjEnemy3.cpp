...
float g_enemy3_x = 150.0f;
float g_enemy3_y = 0.0f;

//位置情報X取得用
float CObjEnemy3::GetX()
{
	return m_px;
}

//位置情報X取得用
float CObjEnemy3::GetY()
{
	return m_py;
}
...
void CObjEnemy3::Init()
{
	...
	//当たり判定用HitBoxを作成
	Hits::SetHitBox(this, m_px, m_py, 45, 60, ELEMENT_ENEMY, OBJ_ENEMY, 1);
}
...
void CObjEnemy3::Draw()
{
	...
	if (m_posture == 2)//下から上
	{
		//切り取り位置の設定
		src.m_top = 205.0f;
		src.m_left = 0.0f + AniData[m_ani_frame] * 62;
		src.m_right = 62.0f + AniData[m_ani_frame] * 62;
		src.m_bottom = 254.0f;
	...
	else if (m_posture == 0)//上から下
	{
		//切り取り位置の設定
		src.m_top = 5.0f;
		src.m_left = 3.0f + AniData[m_ani_frame] * 62;
		src.m_right = 62.0f + AniData[m_ani_frame] * 62;
		src.m_bottom = 62.0f;
