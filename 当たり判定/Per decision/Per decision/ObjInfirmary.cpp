...
float CObjInfirmary::Dot(float ax, float ay, float bx, float by)
{
	float t = 0.0f;
...
//戻り値 float :射影
//内容 AベクトルとBベクトルで内積を行い射影を求める
float CObjInfirmary::Cross(float ax, float ay, float bx, float by)
{
	float t = 0.0f;
...
//主人公と壁の交差判定関数
bool CObjInfirmary::HeroInfirmaryCrossPoint(
...
//InfirmaryHit関数
void CObjInfirmary::InfirmaryHit
(
